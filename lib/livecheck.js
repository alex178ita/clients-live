...
async function checkClient({ key, candidates }, { force = false } = {}) {
  ...
  // The Website in the CRM can carry a locale path — us.mcmworldwide.com/en_US/home,
  // candy-home.com/it_IT/ — and that path is the page under contract. The root
  // of such a site is usually a country picker, so it may answer 200 with no
  // Kleecks marker on it while the real page is perfectly live. Candidates that
  // keep the original path can therefore settle the question either way; the
  // root fallback may only ever promote a row to "live", never condemn it to
  // "offline". Otherwise a multi-country client reads offline for a reason that
  // has nothing to do with Kleecks.
  const wanted = pathOf(candidates[0]);
  ...
  for (const url of candidates) {
    ...
    if (result.status === 'offline' && pathOf(url) === wanted) {
      offline = result;
      break;
    }
    if (!blind && result.status === 'unknown') blind = result;
  }

  const last =
    live ||
    offline ||
    blind || { status: 'unknown', signals: [], reason: 'the page in the CRM could not be read' };

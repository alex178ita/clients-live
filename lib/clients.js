...
// A renewal still being signed. Such a deal never builds a row of its own — the
// client list stays a list of won licences — but a client whose licence has run
// out while its renewal sits in Closing is not a client who left, and showing it
// as plainly "expired" misreads the situation.
const CLOSING_STAGE = process.env.CLOSING_STAGE || '6. Closing';
...
function buildQuery() {
  return (
    ...
    `where Licence > 0 and (Stage = '${WON_STAGE}' or Stage = '${CLOSING_STAGE}') ` +
    `order by id asc`
  );
}
...
async function fetchWonClients() {
  ...
  for (const raw of deals) {
    ...
    if (raw.Stage === CLOSING_STAGE) {
      entry.closing.push(deal);
      continue;
    }
  ...
  for (const entry of byClient.values()) {
    // Only closing deals and no won one: not a client yet. Skip it.
    ...
    rows.push({
      ...
      renewalPending: entry.closing.length > 0,
      renewalEnd: entry.closing.map((d) => d.licenceEnd).filter(Boolean).sort().pop() || null,
